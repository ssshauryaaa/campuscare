...
// ── All localStorage keys used by CampusCare CTF ─────────────────────────────
const RESET_KEYS = [
  { key: "campus_patched_vulns",      label: "Patched Vulnerabilities",     icon: "🛡️", category: "defense" },
  { key: "campus_attack_log",         label: "Real Attack Log",              icon: "⚡", category: "defense" },
  { key: "campuscare_scan_startTs",   label: "Vulnerability Scanner State",  icon: "🔍", category: "scanner" },
  { key: "campuscare_scan_done",      label: "Scanner Completion Flag",      icon: "✅", category: "scanner" },
  { key: "campuscare_scan_bonus_given", label: "Scanner Bonus Award",        icon: "🏆", category: "scanner" },
  { key: "jwt",                       label: "Active User JWT Token",        icon: "🔑", category: "session" },
];

type Category = "defense" | "scanner" | "session" | "all";
...
const CATEGORY_COLORS: Record<Category, { text: string; bg: string; border: string; glow: string }> = {
  ...
  scanner: { text: "#60a5fa", bg: "rgba(96,165,250,0.08)", border: "rgba(96,165,250,0.25)", glow: "rgba(96,165,250,0.15)" },
        ...
        <div style={{ width: 400, opacity: mounted ? 1 : 0, transform: mounted ? "none" : "translateY(12px)", transition: "all 0.5s" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: "#ffffff", marginBottom: 4 }}>
              CampusCare <span style={{ color: "#f5820a" }}>Admin Reset</span>
            </div>
          ...
          <div style={{ marginTop: 16, textAlign: "center", fontSize: 11, color: "rgba(255,255,255,0.2)", fontFamily: mono }}>
            This page is not linked in the main UI. Access via /admin/reset
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: "100vh", background: "#0d1117", fontFamily: sans, color: "#e2e8f0", fontSize: 13 }}>
      ...
      <header style={{ padding: "0 32px", height: 58, background: "#161b22", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>CampusCare <span style={{ color: "#f5820a" }}>Event Control</span></div>
      ...
      <main style={{ padding: "28px 32px", maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", fontFamily: mono }}>
                    Clears ALL CTF state — patches, logs, scanner, sessions. Gives teams a completely fresh start.
                  </div>
                </div>
                {confirm === "all" ? (
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            ...
            {/* Category resets */}
            {(["defense", "scanner", "session"] as Category[]).map(cat => {
