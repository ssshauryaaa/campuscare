...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    ...
    // DEFENSE INTERCEPTION: If patched, block directory traversal attempts on the frontend
    if (lfiPatched && (fileParam.includes("../") || fileParam.includes("..\\"))) {
      if (typeof window !== "undefined" && (window as any).__logCampusAttack) {
        (window as any).__logCampusAttack({
          type: "lfi_documents",
          severity: "critical",
          detail: "Blocked LFI attempt (Directory Traversal)",
          endpoint: `/documents?file=${fileParam}`,
          payload: fileParam,
        });
      }
    ...
    // Log the real attack if it looks like LFI
    if (fileParam.includes("../") || fileParam.includes("..\\")) {
      if (typeof window !== "undefined" && (window as any).__logCampusAttack) {
        (window as any).__logCampusAttack({
          type: "lfi_documents",
          severity: "critical",
          detail: "LFI attempt detected (Directory Traversal)",
          endpoint: `/documents?file=${fileParam}`,
          payload: fileParam,
        });
      }
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--cc-bg)" }}>
      ...
      <div style={{ flex: 1, padding: "40px 60px", marginLeft: 240, marginTop: 56 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 30 }}>
          <div>
            <h1 style={{ fontSize: 32, fontWeight: 800, color: "var(--cc-navy)", margin: "0 0 8px 0" }}>
              Campus Documents
            </h1>
            <p style={{ color: "var(--cc-text-muted)", margin: 0, fontSize: 15 }}>
              Official policies, handbooks, and public records.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 30 }}>
              ...
              return (
                <a
                  ...
                  style={{
                    ...
                    borderRadius: 12,
                    color: active ? "var(--cc-navy)" : "var(--cc-text)",
                    textDecoration: "none",
          ...
          <div style={{ flex: 1, background: "#fff", border: "1px solid var(--cc-border)", borderRadius: 12, display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 500 }}>
            <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--cc-border)", background: "#fafafa", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
