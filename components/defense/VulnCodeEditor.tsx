...
export function VulnCodeEditor({ type, onClose, onApply }: Props) {
    ...
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }}>
        <div style={{ background: "#fff", padding: 28, borderRadius: 14, fontFamily: sans, boxShadow: "0 10px 30px rgba(0,0,0,0.15)", maxWidth: 400 }}>
          ...
          <p style={{ margin: "0 0 20px", color: "#6b7280", fontSize: 13, lineHeight: 1.6 }}>A code sample for this vulnerability type hasn't been added yet. The patch will still be recorded.</p>
  ...
  // All files must have been addressed (non-empty fix entered) to apply
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(15, 25, 50, 0.6)", backdropFilter: "blur(8px)" }}>
      <div style={{ background: "#0f172a", width: "96%", maxWidth: 1100, maxHeight: "92vh", borderRadius: 14, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 25px 60px rgba(0,0,0,0.5)", animation: "ideIn 0.25s ease-out forwards", border: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        <div style={{ padding: "0 20px", background: "#1e293b", display: "flex", alignItems: "center", gap: 12, height: 44, flexShrink: 0, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {["#ef4444", "#f59e0b", "#22c55e"].map((c, i) => (
              <div key={i} onClick={i === 0 ? onClose : undefined} style={{ width: 12, height: 12, borderRadius: "50%", background: c, cursor: i === 0 ? "pointer" : "default", opacity: 0.9 }} />
            ))}
          </div>
          <div style={{ flex: 1, textAlign: "center" }}>
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", fontFamily: mono }}>
              CampusCare IDE  ·  <span style={{ color: "#f5820a" }}>{vuln.title}</span>
            </span>
          </div>
        ...
        <div style={{ display: "flex", flex: 1, minHeight: 0, overflow: "hidden" }}>
          ...
          {/* Left: Info panel */}
          <div style={{ width: 260, background: "#1e293b", display: "flex", flexDirection: "column", borderRight: "1px solid rgba(255,255,255,0.07)", flexShrink: 0 }}>
            <div style={{ padding: "18px 18px 0", flex: 1, overflowY: "auto" }}>
              ...
              <p style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.65, margin: "0 0 20px" }}>{vuln.description}</p>
              ...
              <div style={{ fontSize: 11, fontFamily: mono, color: "#f5820a", background: "rgba(245,130,10,0.1)", padding: "6px 10px", borderRadius: 6, marginBottom: 20, wordBreak: "break-all", border: "1px solid rgba(245,130,10,0.2)" }}>
                {activeFile.path}
              </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
            ...
            {/* Split / Fix only panes */}
            <div style={{ flex: 1, display: "flex", minHeight: 0 }}>

              {/* Vulnerable code (left of split, hidden in fix-only view) */}
              {view === "split" && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", borderRight: "1px solid rgba(255,255,255,0.07)", minWidth: 0 }}>
                  <div style={{ padding: "8px 16px", background: "rgba(239,68,68,0.08)", borderBottom: "1px solid rgba(239,68,68,0.15)", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: "#f87171", letterSpacing: ".08em" }}>⛔ VULNERABLE CODE</span>
                    <span style={{ fontSize: 10, color: "#64748b", fontFamily: mono, marginLeft: "auto" }}>{activeFile.label}</span>
                  </div>
                  <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
                    <pre style={{
                      margin: 0, color: "#fca5a5", fontSize: 13, fontFamily: mono,
                      whiteSpace: "pre-wrap", wordBreak: "break-all", lineHeight: 1.7,
                      background: "rgba(239,68,68,0.05)", borderRadius: 8, padding: 14,
                      border: "1px solid rgba(239,68,68,0.1)",
                    }}>
                      {activeFile.vulnerableSnippet}
                    </pre>
                  </div>
                </div>
              )}
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
                <div style={{ padding: "8px 16px", background: hasChanged ? "rgba(34,197,94,0.08)" : "rgba(255,255,255,0.03)", borderBottom: `1px solid ${hasChanged ? "rgba(34,197,94,0.2)" : "rgba(255,255,255,0.07)"}`, display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: hasChanged ? "#4ade80" : "#64748b", letterSpacing: ".08em" }}>
                    {hasChanged ? "✅ YOUR FIX" : "✎ YOUR FIX — Edit the code below"}
                  </span>
                  {!hasChanged && (
                ...
                <textarea
                  ...
                  placeholder={`// Paste your fix here...\n// Hint: use parameterized queries, safe rendering, etc.`}
                  spellCheck={false}
                  style={{
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              ...
              style={{
                ...
                opacity: !allAddressed && !loading ? 0.5 : 1,
                transition: "all 0.2s",
              }}
            >
              {loading ? (
